		...
		<div id="list-quote" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="quotePrice" title="${message(code: 'quote.quotePrice.label', default: 'Quote Price')}" />
				...
				<g:each in="${quoteInstanceList}" status="i" var="quoteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${quoteInstance.id}">${fieldValue(bean: quoteInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: quoteInstance, field: "quotePrice")}</td>
